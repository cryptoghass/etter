...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ETTERMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ETTERMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ETTERMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("etter-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("ETTERMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running ETTERMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadETTERMiner() exception");
    } catch (...) {
        LogPrintf("ThreadETTERMiner() exception");
    }

    LogPrintf("ThreadETTERMiner exiting\n");
}
